...
from sumy.summarizers.text_rank import TextRankSummarizer as Summarizer5
...
def generate_text_rank_summary(file_name, top_n):
    #url = "https://en.wikipedia.org/wiki/Automatic_summarization"
    #parser = HtmlParser.from_url(url, Tokenizer(LANGUAGE))
    # or for plain text files
    parser = PlaintextParser.from_file(file_name, Tokenizer(LANGUAGE))
    ...
    print(final_text)
    return final_text
generate_text_rank_summary("text.txt", 5)